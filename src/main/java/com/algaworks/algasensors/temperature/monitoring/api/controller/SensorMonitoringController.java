...
import com.algaworks.algasensors.temperature.monitoring.api.mapper.SensorMonitoringMapper;
...
public class SensorMonitoringController {
    ...
    private final SensorMonitoringMapper mapper;

    @GetMapping
    public SensorMonitoringOuput getDetail(@PathVariable TSID sensorId) {
        SensorMonitoring sensorMonitoring = findByIdOrDefault(sensorId);
        return mapper.sensorMonitoringToSensorMonitoringOuput(sensorMonitoring);
    }
    ...
    public void disable(@PathVariable TSID sensorId) {
        SensorMonitoring sensorMonitoring = findByIdOrDefault(sensorId);
        mockTimeoutException(sensorMonitoring);
        sensorMonitoring.setEnabled(false);
        sensorMonitoringRepository.saveAndFlush(sensorMonitoring);
    }

    @SneakyThrows
    private void mockTimeoutException(SensorMonitoring sensorMonitoring) {
        if (!sensorMonitoring.getEnabled()) {
